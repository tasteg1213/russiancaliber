                ...
                if(CModule::IncludeModule("iblock")){
                    ...
                    if(!empty($arElement)){
                        ...
                        $arMessage = array(
                            ...
                            "CLIENT_MC_ID" => htmlspecialcharsbx($_GET["CLIENT_MC_ID"])
                        );

//						CEvent::SendImmediate(
//						        "SALE_DRESSCODE_REQUEST_SEND",
...
//                        );
        ...
        elseif($_GET["act"] == "addCart"){
            ...
            if(!empty($_GET["multi"]) && !empty($_GET['id'])){
                ...
                $arGroupClock = array(11);
                $title_quantity = '????? ? ???????';
                $db_old_groups = CIBlockElement::GetElementGroups($_GET['id'], true);
                while($ar_group = $db_old_groups->Fetch()){
                    if (array_search($ar_group["ID"], $arGroupClock) !== false)
                        $title_quantity = '????? ??? ?????';
                }
