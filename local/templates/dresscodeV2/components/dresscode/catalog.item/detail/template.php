                ...
                <? if (!empty($arResult["TABS"])): ?>
                    <div class="tabs changeTabs">
                        <? foreach ($arResult["TABS"] as $it => $arTab): ?>
                            <?if ($it == "CATALOG_ELEMENT_BACK" && !empty($arTab["LINK"]))
                                $arTab["LINK"] = $arTab["LINK"]."?em=".$arResult["ID"];
//                                $arTab["LINK"] = $arTab["LINK"]."#".$arResult["ID"];?>
                ...
                <div id="elementContainer" class="column">
                    <div class="mainContainer" id="browse">
                        <div class="col">
                            ...
                            <? endif; ?>
                            <div class="wishCompWrap topCompare">
                                            ...
                                            <div class="item">
                                                <a href="<?= $arNextPicture["LARGE_IMAGE"]["SRC"] ?>"
                                                   ...
                                                   data-large-picture="<?= $arNextPicture["LARGE_IMAGE"]["SRC"] ?>"><img
                                                            ...
                                                            alt="<? if ($ipr == 0): ?><? if (!empty($arResult["IPROPERTY_VALUES"]["ELEMENT_DETAIL_PICTURE_FILE_ALT"])): ?><?= $arResult["IPROPERTY_VALUES"]["ELEMENT_DETAIL_PICTURE_FILE_ALT"] ?><? else: ?><?= $arResult["NAME"] ?><? endif; ?><? endif; ?>"
                                                            title="<? if ($ipr == 0): ?><? if (!empty($arResult["IPROPERTY_VALUES"]["ELEMENT_DETAIL_PICTURE_FILE_TITLE"])): ?><?= $arResult["IPROPERTY_VALUES"]["ELEMENT_DETAIL_PICTURE_FILE_TITLE"] ?><? else: ?><?= $arResult["NAME"] ?><? endif; ?><? endif; ?>"></a>
                                            </div>
                                        <? endforeach; ?>
                                    </div>
                                </div>
                                ...
                                <? if ($arParams["SHOW_REVIEW_FORM"]): ?>
                                    <div class="row">
                                        <a href="#" class="reviewAddButton label"><img
                                                    src="<?= SITE_TEMPLATE_PATH ?>/images/addReviewSmall.png"
                                                    alt="<?= GetMessage("REVIEWS_ADD") ?>" class="icon"><span
                                                    class="labelDotted"><?= GetMessage("REVIEWS_ADD") ?></span></a>
                                    </div>
                                <? endif; ?>
                                        ...
                                        <? if (!empty($arNextProp["VALUES"])): ?>
                                            <? if ($arNextProp["LIST_TYPE"] == "L" && $arNextProp["HIGHLOAD"] != "Y"): ?>
                                                <div class="elementSkuProperty elementSkuDropDownProperty"
                                                    ...
                                                    <div class="elementSkuPropertyName"><?= $arNextProp["NAME"] ?>:
                                                    </div>
                    ...
                    <? if ($arResult["SHOW_RELATED"] == "Y"): ?>
                        <div id="related">
                            <div class="heading"><?= GetMessage("CATALOG_ELEMENT_ACCEESSORIES") ?>
                                (<?= $arResult["RELATED_COUNT"] <= 8 ? $arResult["RELATED_COUNT"] : 8 ?>)
                            </div>
                            <? $APPLICATION->IncludeComponent(
	"dresscode:catalog.section", 
	"squares", 
                    ...
                    <? if ($arResult["SHOW_SIMILAR"] == "Y"): ?>
                        <div id="similar">
                            <div class="heading"><?= GetMessage("CATALOG_ELEMENT_SIMILAR") ?>
                                (<?= $arResult["SIMILAR_COUNT"] <= 8 ? $arResult["SIMILAR_COUNT"] : 8 ?>)
                            </div>
                            <? $APPLICATION->IncludeComponent(
	"dresscode:catalog.section", 
	"squares", 
...
<?
global $USER;
$AjaxParamsModelRad = array(
    "IBLOCK_ID_MODEL" => $arParams["IBLOCK_ID_MODEL"],
    "TEXT_NAME_MODEL" => $arParams["TEXT_NAME_MODEL"],
    "PRODUCT_ID" => $arParams["PRODUCT_ID"],
    "IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
    "IBLOCK_ID" => $arParams["IBLOCK_ID"],
    "PRODUCT_PRICE_CODE" => $arParams["PRODUCT_PRICE_CODE"],

    // "DETAIL_PROPERTY_CODE" => $arResult["MODEL_LINE_PROPERTIES_LIST"]
    "DETAIL_PROPERTY_CODE" => $arResult["MODEL_LINE_PROPERTIES_LIST"]
    // "DETAIL_PROPERTY_CODE" => array_merge($arParams["DETAIL_PROPERTY_CODE"],array_keys($arResult["PROPERTIES"]))
    //"DETAIL_PROPERTY_CODE" => array_keys($arResult["PROPERTIES"])
);


// if ($USER->IsAdmin()) {
//     echo "<pre>";
//     print_r($arResult["MODEL_LINE_PROPERTIES_LIST"]);
//     print_r(array_merge($arParams["DETAIL_PROPERTY_CODE"],array_keys($arResult["PROPERTIES"])));
//     echo "</pre>";
// }

$AjaxParamsModelRad = base64_encode(gzcompress(serialize($AjaxParamsModelRad)));
?>
    ...
    <script type="text/javascript">
        ...
        var AjaxPathModelRad = "<?=$templateFolder . "/ajax_model_rad.php"?>";
        var AjaxParamsModelRad = "<?= $AjaxParamsModelRad?>";
        var AjaxParamsModelRadText = "<?= $arParams["TEXT_NAME_MODEL"]?>";
