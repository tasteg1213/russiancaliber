...
$(document).on("click", ".one_param", function() {
	var id = $(this).attr('id');
	$("input#"+id).click();
    BX.addCustomEvent('onAjaxSuccess', function(){
        $('#modef').hide();
        window.location.href = $('a.modef_send_link').attr('href');
    });
});
$(document).on("click", "#one_param_cena", function() {
    var id_min = $(this).data( "min" );
    var id_max = $(this).data( "max" );
    var min_val = $(this).data( "min_value" );
    var max_val = $(this).data( "max_value" );
    $("#"+id_min).val(min_val);
    $("#"+id_max).val(max_val);
    $("#"+id_max).change();
});
...
function mobFilterWidthResize (e) {
	if (window.outerWidth<=1024) {
		$("#smartFilter").css({"padding-right": 80});
	};
}

function filterPreload(fparam = false) {
	switch(fparam) {
		case 'create':
			if ($(".preloader").length == 0) {
				$("#smartFilter").prepend("<div class='preloader'></div>");
				// console.log("create");
			}
		...
		case 'show':
			$("#smartFilter .preloader").addClass("filterPreloader");
			// console.log("show");
			break;

		case 'hide':
			$("#smartFilter .preloader").removeClass("filterPreloader");
			// console.log("hide");
			break;
