...
<?
$this->SetViewTarget("canonical_detail");
...
global $DB;
$bd_info = $DB->Query(
    'SELECT NEW_URL FROM b_sotbit_seometa_chpu WHERE REAL_URL = "'.$_SERVER['REQUEST_URI'].'"'
);
$url_bd = $bd_info->Fetch()['NEW_URL'];
if ( $url_bd )
    $url = $url_bd;
...
<div id="catalogColumn">
	...
	<div class="rightColumn">
		...
		<?if(!empty($_REQUEST["SORT_FIELD"]) && !empty($arSortFields[$_REQUEST["SORT_FIELD"]])){
			...
			$arParams["ELEMENT_SORT_FIELD2"] = $arSortFields[$_REQUEST["SORT_FIELD"]]["CODE"];
			$arParams["ELEMENT_SORT_ORDER2"] = $arSortFields[$_REQUEST["SORT_FIELD"]]["ORDER"];
			...
			$arParams["ELEMENT_SORT_FIELD2"] = $arSortFields[$_COOKIE["CATALOG_SORT_FIELD"]]["CODE"];
			$arParams["ELEMENT_SORT_ORDER2"] = $arSortFields[$_COOKIE["CATALOG_SORT_FIELD"]]["ORDER"];
		...
		<div id="catalog">
            ...
            global $DB;
            $res = $DB->Query(
                    "select * from b_sotbit_seometa_chpu where NEW_URL='".$APPLICATION->GetCurPage()."'",
                    false
            );
            if (empty($res->Fetch())){?>
            ...
            <?
                ...
                $APPLICATION->ShowViewContent('filter_block_one');
	...
	false
            ...
            <div class="catalog_list_seometa">
                <?php
            ...
            $ajax = false;
                if ($request->isPost()) {
                    ...
                    $cook = $request->getCookie("ELMENTS_ON_PAGE");
                    $ajax = true;
                    if (!empty($cook) && $cook+0 > 0)
                        $arParams["PAGE_ELEMENT_COUNT"] = $cook;
...
//                    "ELEMENT_SORT_ORDER" => "asc,nulls",
					...
					"PROPERTY_CODE" => $ajax?$arParams["PROPERTY_CODE"]:$arParams["LIST_PROPERTY_CODE"],
            ...
            <div class="catalog_list_seometa">
                <?php
                global $sotbitSeoMetaBottomDesc;//для установки верхнего описания
                echo $sotbitSeoMetaBottomDesc;//вывод верхнего описания
                ?>
            </div>
		</div>
	</div>
