					...
					<tbody>
						...
						<?foreach ($arParams["PRODUCT_DISPLAY_PROPERTIES"] as $nextPropCode){?>
							<?if(!empty($arResult["DISPLAY_PROPERTIES"][$nextPropCode]["DISPLAY_VALUE"]) && $arResult["DISPLAY_PROPERTIES"][$nextPropCode]["SORT"] <= 5000){?>
								...
								<tr>
                                    <?
                                        if($arResult["DISPLAY_PROPERTIES"][$nextPropCode]["MULTIPLE"] == "Y" && ($arResult["DISPLAY_PROPERTIES"][$nextPropCode]["PROPERTY_TYPE"] == "S" || $arResult["DISPLAY_PROPERTIES"][$nextPropCode]["PROPERTY_TYPE"] == "N")){?>
                                            <td><span><?=preg_replace("/\[.*\]/", "", $arResult["DISPLAY_PROPERTIES"][$nextPropCode]["NAME"])?></span></td>
                                            <td>
                                                <?foreach ($arResult["DISPLAY_PROPERTIES"][$nextPropCode]["DISPLAY_VALUE"] as $key => $disp_val_for){?>
                                                <?=$disp_val_for?><br>
                                                <?}?>
                                            </td>
                                        <?}else{?>
			...
			<div class="resizeColumn last">
				<div class="optional">
					<div class="row">
						...
						<?if(empty($arResult["PRICE"]) || $arResult["CATALOG_AVAILABLE"] != "Y" || $arResult["CATALOG_QUANTITY"] < 1):?>
						disabled<?endif;?>" data-id="<?=$arResult["ID"]?>"><img src="<?=SITE_TEMPLATE_PATH?>/images/fastBack.png" alt="" class="icon">
                            <?=GetMessage("FASTBACK_LABEL")?></a>
