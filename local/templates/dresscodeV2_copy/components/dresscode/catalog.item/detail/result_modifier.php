...
$this->SetViewTarget("canonical_detail");
    ...
    if($element['DETAIL_PAGE_URL'])
        echo '<link rel="canonical" href="'.$element['DETAIL_PAGE_URL'].'"/>';
$this->EndViewTarget();
...
// if ($USER->IsAdmin()) {
	...
	$arFilter = Array("ID" => $arResult["ID"],"IBLOCK_ID"=>IntVal($arResult["IBLOCK_ID"]), "ACTIVE"=>"Y");
	$res = CIBlockElement::GetList(Array(), $arFilter, false, false, $arSelect);
	
	if($ob = $res->GetNextElement()){
		$arProps = $ob->GetProperties();
	    foreach ($arProps as $propVal) {
	    	if ($propVal["SORT"] < 5000) {
				$tmpProps[] = $propVal["CODE"];
			}
		}
	}

	$arResult["MODEL_LINE_PROPERTIES_LIST"] = $tmpProps;
// }
