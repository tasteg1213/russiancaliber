    ...
    <div id="personalCart">
        ...
        <div id="basketTopLine">
            <div id="tabsControl">
                ...
                <div class="item"><a href="<?= SITE_DIR ?>personal/cart/order/" id="scrollToOrder"
                                     class="selected"><?= GetMessage("BASKET_TABS_ORDER_MAKE") ?></a></div>
            ...
            <? if (!empty($arBasketTemplates)): ?>
                <div id="basketView">
                    ...
                    <? foreach ($arBasketTemplates as $arNextBasketTemplate): ?>
                        <div class="item">
                            <a href="<?= $arNextBasketTemplate["CHANGE_URL"] ?>"
                               class="<?= $arNextBasketTemplate["CLASS_NAME"] ?><? if ($arNextBasketTemplate["SELECTED"] == "Y"): ?> selected<? endif; ?>"></a>
                        </div>
                    <? endforeach; ?>
                </div>
            <? endif; ?>
...
*/ ?>
        ...
        <div id="order" class="orderContainer">
                            ...
                            <select id="personSelect">
                                <? foreach ($arResult["PERSON_TYPE"] as $arPersonType): ?>
                                    <? if ($arPersonType["ACTIVE"] === "Y"): ?>
                                        <option value="<?= $arPersonType["ID"] ?>"
                                                data-id="<?= $arPersonType["ID"] ?>"><?= $arPersonType["NAME"] ?></option>
                                    <? endif; ?>
                                <? endforeach; ?>
                            </select>
                        <? endif; ?>
            ...
            <? if (!empty($arResult["PERSON_TYPE"])): ?>
                <? foreach ($arResult["PERSON_TYPE"] as $personIndex => $arPersonType): ?>
                    <form id="orderForm_<?= $arPersonType["ID"] ?>">
                        <table class="orderProps<? if ($arPersonType["FIRST"] == "Y"): ?> active<? endif; ?>"
                               data-id="person_<?= $arPersonType["ID"] ?>">
                                        ...
                                        <td>
                                            <ul class="userProp">
                                                <? foreach ($arResult["PROPERTIES"][$groupIndex] as $arProperty): ?>
                                                    ...
                                                    <li <?= ($arProperty["TYPE"] == "LOCATION") ? "class='hide_location'" : "" ?>>
                                                        ...
                                                        <? if ($arProperty["TYPE"] == "TEXT"): ?>
                                                            <input type="text"
                                                                   name="ORDER_PROP_<?= $arProperty["ID"] ?>"
                                                                   value="<?= $arProperty["DEFAULT_VALUE"] ?>"
                                                                   <? if ($arProperty["IS_EMAIL"] === "Y"): ?>data-mail="Y"<? endif; ?>
                                                                   data-requied="<? if ($arProperty["REQUIED"] === "Y"): ?>Y<? endif; ?>"
                                                                   id="<?= $arProperty["ID"] ?>"<? if ($arProperty["IS_PAYER"] == "Y"): ?> data-payer="Y"<? endif ?><? if ($arProperty["IS_PHONE"] == "Y"): ?> data-mobile="Y"<? endif ?>>
                                                        <? elseif ($arProperty["TYPE"] == "LOCATION"): ?>
                                                            <input type="text"
                                                                   name="ORDER_PROP_<?= $arProperty["ID"] ?>"
                                                                   value="<?= $arProperty["DEFAULT_VALUE"] ?>"
                                                                   data-requied="<? if ($arProperty["REQUIED"] === "Y"): ?>Y<? endif; ?>"
                                                                   id="<?= $arProperty["ID"] ?>"
                                                                   class="location"<? if (!empty($arProperty["LOCATION_ID"])): ?> autocomplete="off" data-location="<?= $arProperty["LOCATION_ID"] ?>"<? endif; ?>>
                                                        <? elseif ($arProperty["TYPE"] == "TEXTAREA"): ?>
                                                            <textarea name="ORDER_PROP_<?= $arProperty["ID"] ?>"
                                                                      value="<?= $arProperty["DEFAULT_VALUE"] ?>"
                                                                      data-requied="<? if ($arProperty["REQUIED"] === "Y"): ?>Y<? endif; ?>"
                                                                      id="<?= $arProperty["ID"] ?>"<? if ($arProperty["IS_ADDRESS"] === "Y"): ?> data-address="Y"<? endif; ?>></textarea>
                                                        <? elseif ($arProperty["TYPE"] == "CHECKBOX"): ?>
                                                            <input type="checkbox"
                                                                   value="Y"<? if ($arProperty["DEFAULT_VALUE"] == "Y"): ?> checked<? endif; ?>
                                                                   data-requied="<? if ($arProperty["REQUIED"] === "Y"): ?>Y<? endif; ?>"
                                                                   id="<?= $arProperty["ID"] ?>" class="electroCheck"
                                                                   data-class="electroCheck_div"
                                                                   name=name="ORDER_PROP_<?= $arProperty["ID"] ?>">
                                                        ...
                                                        <? elseif ($arProperty["TYPE"] == "RADIO"): ?>
                                                            ...
                                                            <? while ($vars = $db_vars->Fetch()): ?>
                                                                <input type="radio"
                                                                       ...
                                                                       value="<?= $vars["VALUE"] ?>"
                                                                       data-requied="<? if ($arProperty["REQUIED"] === "Y"): ?>Y<? endif; ?>"
                                                                       id="<?= $arProperty["ID"] ?>" <?= (($vars["VALUE"] == $arProperty["DEFAULT_VALUE"]) ? " checked" : "") ?>>
                                                                <label for="<?= $arProperty["ID"] ?>"><?= htmlspecialchars($vars["NAME"]) ?></label>
                                                            <? endwhile; ?>
                                    ...
                                    <td>
                                        ...
                                        <select class="deliSelect" name="DEVIVERY_TYPE">
                                            <? foreach ($arResult["DELIVERY"][$arPersonType["ID"]] as $arDevivery): ?>
                                                ...
                                                <? endif; ?>
                                                <option data-price="<?= intval($arDevivery["PRICE"]) ?>"
                                                        value="<?= $arDevivery["ID"] ?>"><?= $arDevivery["NAME"] ?> <?= str_replace("-", ".", CurrencyFormat($arDevivery["PRICE"], $arDevivery["CURRENCY"])) ?></option>
                                            <? endforeach; ?>
                                        </select>
                                        <? if (!empty($arResult["DELIVERY_PROPS"])): ?>
                                            <ul class="userProp">
                                                <? foreach ($arResult["DELIVERY_PROPS"] as $i => $arProperty): ?>
                                                    ...
                                                    <li data-id="deli_<?= $arProperty["DELIVERY_ID"] ?>"
                                                        class="deliProps<? if (!empty($visibile)): ?> hidden<? endif; ?>">
                                                        <? if ($arProperty["TYPE"] != "CHECKBOX"): ?>
                                                            <span class="label"><?= $arProperty["NAME"] ?><? if ($arProperty["REQUIED"] === "Y"): ?>*<? endif; ?></span>
                                                            <label><?= $arProperty["DESCRIPTION"] ?></label>
                                                        <? endif; ?>
                                                        <? if ($arProperty["TYPE"] == "TEXT"): ?>
                                                            <input type="text"
                                                                   ...
                                                                   value="<?= $arProperty["DEFAULT_VALUE"] ?>"
                                                                   data-requied="<? if ($arProperty["REQUIED"] === "Y"): ?>Y<? endif; ?>"
                                                                   id="<?= $arProperty["ID"] ?>" <?= $visibile ?>>
                                                        <? elseif ($arProperty["TYPE"] == "TEXTAREA"): ?>
                                                            <textarea name="ORDER_PROP_<?= $arProperty["ID"] ?>"
                                                                      value="<?= $arProperty["DEFAULT_VALUE"] ?>"
                                                                      data-requied="<? if ($arProperty["REQUIED"] === "Y"): ?>Y<? endif; ?>"
                                                                      id="<?= $arProperty["ID"] ?>" <? if ($arProperty["IS_ADDRESS"] === "Y"): ?> data-address="Y"<? endif; ?> <?= $visibile ?>><? if ($arProperty["IS_ADDRESS"] === "Y"): ?><?= $arResult["USER"]["PERSONAL_STREET"] ?><? endif; ?></textarea>
                                                        <? elseif ($arProperty["TYPE"] == "LOCATION"): ?>
                                                            <input type="text"
                                                                   name="ORDER_PROP_<?= $arProperty["ID"] ?>"
                                                                   value="<?= $arProperty["DEFAULT_VALUE"] ?>"
            ...
            <div class="orderLine bottom">
                <div id="sum">
                    <a href="#" class="order" id="orderMake"><img
                                src="<?= SITE_TEMPLATE_PATH ?>/images/order.png"> <?= GetMessage("ORDER_GO") ?></a>
                    <span class="label hd"><?= GetMessage("TOTAL_QTY") ?></span> <span class="price hd"
                                                                                       id="countOrderItems"><?= $countPos ?></span>
                    <span class="label"><?= GetMessage("ORDER_DELIVERY") ?>:</span>
                    <span class="price"><span
                                id="allDevilerySum"><?= FormatCurrency($arResult["FIRST_DELIVERY_PRICE"], $OPTION_CURRENCY); ?></span></span>
                    <span class="label"><?= GetMessage("TOTAL_SUM") ?></span>
                    <span class="price"><span
                                id="allOrderSum"><?= FormatCurrency($arResult["SUM"] + $arResult["FIRST_DELIVERY_PRICE"], $OPTION_CURRENCY); ?></span></span>
                </div>
            </div>
        </div>
        <div class="clear"></div>
