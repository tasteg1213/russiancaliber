	...
	if (typeof ym == 'function') {
		ym(53244133, 'getClientID', function (clientID) {
			if ($("#personalCart").length > 0) {
				...
				var clientIdLine = $("input[name*='ORDER_PROP_"+el+"']");
				clientIdLine.parent().css({"display":"none"});
				clientIdLine.val(clientID);
				clientIdLine.attr({"value": clientID});
			}
		});
	}
};
