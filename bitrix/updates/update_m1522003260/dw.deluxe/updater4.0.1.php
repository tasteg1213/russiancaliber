	...
	while ($arSite = $rsSites->Fetch()){
		if (is_dir($arSite["ABS_DOC_ROOT"])){
			...
			while($rsTempl = $rsTemplates->Fetch()){
				if(strstr($rsTempl["TEMPLATE"], "dresscode")){
					$CURRENT_IBLOCK_ID = 0;
					$CURRENT_IBLOCK_TYPE_ID = 0;
					$CURRENT_VERSION = "";
					if($rsTempl["TEMPLATE"] == "dresscodeV2"){
						$CURRENT_VERSION = "V2";
					}
					...
					while($arIblock = $rsIblock->Fetch()){
						$arCatalogIblock = CCatalog::GetByID($arIblock["ID"]);
						if(!empty($arCatalogIblock)){
							if($arCatalogIblock["OFFERS"] == "N"){
								$CURRENT_IBLOCK_ID = $arCatalogIblock["IBLOCK_ID"];
								$CURRENT_IBLOCK_TYPE_ID = $arCatalogIblock["IBLOCK_TYPE_ID"];
							}
						}
					}

					if(!empty($CURRENT_IBLOCK_ID)){
						$tmpSectFooter = file_get_contents(dirname(__FILE__).'/install/files/sect_footerTabs.php');
						if(!empty($tmpSectFooter)){
							$tmpSectFooter = str_replace("#CURRENT_IBLOCK_ID#", $CURRENT_IBLOCK_ID, $tmpSectFooter);
							$tmpSectFooter = str_replace("#CURRENT_IBLOCK_TYPE_ID#", $CURRENT_IBLOCK_TYPE_ID, $tmpSectFooter);
							$tmpSectFooter = str_replace("#CURRENT_VERSION#", $CURRENT_VERSION, $tmpSectFooter);
						}else{
							echo dirname(__FILE__).'/install/files/sect_footerTabs.php';
						}
					}

					file_put_contents(dirname(__FILE__).'/install/wizards/dw/deluxe/site/public/ru/sect_footerTabs.php', $tmpSectFooter);
					$updater->CopyFiles("install/wizards/dw/deluxe/site/public/ru", $arSite["DIR"]);
				}
			}
		}
	}
