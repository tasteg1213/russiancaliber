                    ...
                    <?if((!empty($arProp["VALUE"]) || !empty($arProp["LINK"])) && $arProp["SORT"] <= 5000):?>
                        ...
                        <?if(preg_match("/\[.*\]/", trim($arProp["NAME"]), $res, PREG_OFFSET_CAPTURE)):?>
                            ...
                            <tr<?if($set % 2):?> class="gray"<?endif;?>>
                                ...
                                ?>
                                <td class="name <?=$dop_class?>">
                                    <span>
                                        <?=preg_replace("/\[.*\]/", "", trim($PROP_NAME))?>
                                        <?if ($arResult['FILTER_HINT'][$arProp['ID']]['FILTER_HINT']){?>
                                            <noindex>
                                                <div class="hint">
                                                    <?/*<span>?</span>*/?>
                                        ...
                                        <?}?>
                                    </span><?if(!empty($arProp["HINT"])):?><a href="#" class="question" title="<?=$arProp["HINT"]?>" data-description="<?=$arProp["HINT"]?>"></a><?endif;?></td>
                                <td>
