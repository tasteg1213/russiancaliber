            ...
            <?foreach ($arResult["DISPLAY_PROPERTIES"] as $ip => $arProperty):?>
                <?if(!empty($arProperty["DISPLAY_VALUE"]) && ++$propertyCounter <= $arParams["COUNT_PROPERTIES"]):?>
                    ...
                    <div class="propertyTable">
                        <? $dop_class = false;
                        if ($arResult['FILTER_HINT'][$arProperty['ID']]['FILTER_HINT'])
                            $dop_class = 'visibleHint';
                        ?>
                        <div class="propertyName <?=$dop_class?>">
                            <?=preg_replace("/\[.*\]/", "", $arProperty["NAME"])?>
                            <?if ($arResult['FILTER_HINT'][$arProperty['ID']]['FILTER_HINT']){?>
                                <noindex>
                                    <div class="hint">
                                        <span>?</span>
                                        <div class="hintValue hint_hide" >
                                            <?=$arResult['FILTER_HINT'][$arProperty['ID']]['FILTER_HINT']?>
