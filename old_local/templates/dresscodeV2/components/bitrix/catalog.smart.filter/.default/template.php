...
<div id="smartFilter">
	...
	<form name="<?=$arResult["FILTER_NAME"]."_form"?>" action="<?=$arResult["FORM_ACTION"]?>" method="GET" id="smartFilterForm">
		...
		foreach($arResult["ITEMS"] as $key => $arItem)
		{
			...
			?>
			<div class="paramsBox">
				<div class="paramsBoxTitle <?if(count($arItem["VALUES"]) == 1){echo'one_row_param';}?>">
                    ...
                    <?if(count($arItem["VALUES"]) == 1){?>
                        <?
                        $span_val = preg_replace("/\[.*\]/", "", $arItem["NAME"]);
                         if(strlen($span_val) > 28){
                             $span_val_cut = explode(" ", $span_val);
                             global $USER;
                             if ($USER->IsAdmin()){
                                 /*echo"<pre>";
                                 print_r($span_val_cut);
                                 echo"</pre>";*/
                             }
                         }
                        ?>
                    ...
                    if(count($arItem["VALUES"]) == 1){?>
                        ...
                        <ul class="checkbox" style="display: inline-block; float: right;">
...
<?$this->SetViewTarget('filter_block_one');?>
    <div class="filter_block_one">
        <?
        // echo"<pre>";
       // print_r($arResult["ITEMS"]);
       // echo"</pre>";
        foreach ($arResult["ITEMS"] as $key => $arItem_block){
            foreach($arItem_block["VALUES"] as $arItem_block_val){
                    if($arItem_block_val["CHECKED"] == 1){?>
                        <div class="one_param" id="<?=$arItem_block_val['CONTROL_ID']?>" name="<?=$arItem_block_val['CONTROL_ID']?>" value="Y">
                            <span><?echo preg_replace("/\[.*\]/", "", $arItem_block["NAME"]);
                            echo ": ";
                            echo $arItem_block_val["VALUE"];?></span>
                        </div>
                        <?
                        $clear_all = 1;
                }
            }
        if(($arItem_block["PRICE"] == 1 && !empty($arItem_block["VALUES"]["MIN"]["HTML_VALUE"])) || ($arItem_block["PRICE"] == 1 && !empty($arItem_block["VALUES"]["MAX"]["HTML_VALUE"]))){?>
            <div class="one_param" id="one_param_cena" data-min="<?=$arItem_block["VALUES"]["MIN"]["CONTROL_ID"]?>" data-min_value="<?=(int)$arItem_block["VALUES"]["MIN"]["VALUE"]?>" data-max="<?=$arItem_block["VALUES"]["MAX"]["CONTROL_ID"]?>" data-max_value="<?=(int)$arItem_block["VALUES"]["MAX"]["VALUE"]?>" value="Y">
                <span><?
                    echo preg_replace("/\[.*\]/", "", $arItem_block["NAME"]);
                    echo ": ";
                    if(!empty($arItem_block["VALUES"]["MIN"]["HTML_VALUE"])){
                        echo CurrencyFormat($arItem_block["VALUES"]["MIN"]["HTML_VALUE"], 'RUB');
                    }else{
                        echo CurrencyFormat($arItem_block["VALUES"]["MIN"]["VALUE"], 'RUB');
                    }
                    echo " - ";
                    if(!empty($arItem_block["VALUES"]["MAX"]["HTML_VALUE"])){
                        echo CurrencyFormat($arItem_block["VALUES"]["MAX"]["HTML_VALUE"], 'RUB');
                    }else{
                        echo CurrencyFormat($arItem_block["VALUES"]["MAX"]["VALUE"], 'RUB');
                    }
                    ?></span>
            </div>
                        <?
        ...
        if($arItem_block["DISPLAY_TYPE"] == "A" && (!empty($arItem_block["VALUES"]["MIN"]["HTML_VALUE"]) || !empty($arItem_block["VALUES"]["MAX"]["HTML_VALUE"])) &&
            (($arItem_block["VALUES"]["MIN"]["HTML_VALUE"] != $arItem_block["VALUES"]["MIN"]["VALUE"]) && ($arItem_block["VALUES"]["MAX"]["HTML_VALUE"] != $arItem_block["VALUES"]["MAX"]["VALUE"]))) {?>
            <div class="one_param range" data-min="<?=$arItem_block["VALUES"]["MIN"]["CONTROL_ID"]?>" data-min_value="<?=(int)$arItem_block["VALUES"]["MIN"]["VALUE"]?>" data-max="<?=$arItem_block["VALUES"]["MAX"]["CONTROL_ID"]?>" data-max_value="<?=(int)$arItem_block["VALUES"]["MAX"]["VALUE"]?>" value="Y">
                <span><?echo preg_replace("/\[.*\]/", "", $arItem_block["NAME"]);
                echo ": ";
                if(!empty($arItem_block["VALUES"]["MIN"]["HTML_VALUE"])) {
                    echo $arItem_block["VALUES"]["MIN"]["HTML_VALUE"];
                }else{
                    echo $arItem_block["VALUES"]["MIN"]["VALUE"];
                }
                echo " - ";
                if(!empty($arItem_block["VALUES"]["MAX"]["HTML_VALUE"])) {
                    echo $arItem_block["VALUES"]["MAX"]["HTML_VALUE"];
                }else{
                    echo $arItem_block["VALUES"]["MAX"]["VALUE"];
                }
                ?></span>
            </div>
        <?
        ...
        ?>
<?if($clear_all == 1){?>
        <a href="javascript:void(0);" data-id="del_filter" class="clear_all"><?=GetMessage("CLEAR_ALL");?></a>
    <?}?>
    </div>
<?$this->EndViewTarget();?>
